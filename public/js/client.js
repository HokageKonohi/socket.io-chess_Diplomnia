...
// Определить доску с точки зрения игрока
    assignSquares();
...
// Инициализируем модальные всплывающие окна
  ...
  var showGameOverMessage = function(type) {
    ...
    switch (type) {
      case 'checkmate-win'  : header.addClass('alert-success').text('Мат'); break;
      case 'checkmate-lose' : header.addClass('alert-danger').text('Мат'); break;
      case 'forfeit-win'    : header.addClass('alert-success').text('Ваш оппонент проиграл игру'); break;
      case 'forfeit-lose'   : header.addClass('alert-danger').text('Вы проиграли игру'); break;
      case 'stalemate'      : header.addClass('alert-warning').text('Безвыходное положение'); break;
    }
