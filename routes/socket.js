...
var join = function(gameID) {
  ...
  if (gameID !== sess.gameID) {
    ...
    this.emit('error', {message: "Вы не можете присоединиться к этой игре"});
    return;
  }
  ...
  if (!game) {
    ...
    this.emit('error', {message: "Игра не найдена"});
    return;
  }
  ...
  if (!result) {
    ...
    this.emit('error', {message: "Невозможно присоединиться к игре"});
    return;
  }
...
var move = function(data) {
  ...
  if (data.gameID !== sess.gameID) {
    console.log('ERROR: Access Denied', debugInfo);
    this.emit('error', {message: "Вы не присоединились к этой игре"});
    return;
  }
  ...
  if (!result) {
    ...
    this.emit('error', {message: "Неверный ход. Повторите попытку."});
    return;
  }
...
var forfeit = function(gameID) {
  ...
  if (!result) {
    ...
    this.emit('error', {message: "Не удалось проиграть игру"});
    return;
  }
